...
export default function GitHubButton({ compact = false, className = '', autoAnimate = true, label = 'Github', onClick }: { compact?: boolean; className?: string; autoAnimate?: boolean; label?: string; onClick?: () => void }): JSX.Element {
  ...
  return (
    <div
      className={`${compact ? "relative inline-block w-32 h-9" : "relative inline-block w-44 h-10 md:w-56 md:h-12"} mx-auto group rounded-2xl overflow-hidden border border-white/40 shadow-[0_0_40px_rgba(255,255,255,0.22)] backdrop-saturate-150 ${className}`}
    >
      <div
        className={compact ? "absolute inset-0 opacity-70 pointer-events-none" : "absolute w-[112%] h-[128%] top-[8%] left-1/2 -translate-x-1/2 filter blur-[18px] opacity-70 pointer-events-none"}
      >
        <span className="absolute inset-0 bg-[#d9d9d9] filter blur-[6.5px]" style={{ borderRadius: 'inherit' }}></span>
        <div className="relative w-full h-full overflow-hidden" style={{ borderRadius: 'inherit' }}>
          <Liquid isHovered={autoAnimate ? true : isHovered} colors={COLORS} />
        </div>
      </div>
      {/* inner glow plate */}
      <div className={compact ? "absolute inset-0" : "absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[40%] w-[92.23%] h-[112.85%] bg-[#3b33a5]/40 filter blur-[7.3px] mix-blend-screen"} style={{ borderRadius: 'inherit' }}></div>
      <div className="relative w-full h-full overflow-hidden" style={{ borderRadius: 'inherit' }}>
        {!compact && <span className="absolute inset-0 bg-[#e9e9ef]" style={{ borderRadius: 'inherit' }}></span>}
        <Liquid isHovered={autoAnimate ? true : isHovered} colors={COLORS} />
        {!compact &&
          [1, 2, 3, 4, 5].map((i) => (
            <span
              key={i}
              className={`absolute inset-0 border-solid border-[3px] border-gradient-to-b from-transparent to-white mix-blend-overlay filter ${i <= 2 ? 'blur-[3px]' : i === 3 ? 'blur-[5px]' : 'blur-[4px]'}`}
              style={{ borderRadius: 'inherit' }}></span>
          ))}
        {/* glass sheen */}
        <span className="pointer-events-none absolute inset-0" style={{ borderRadius: 'inherit', background: 'linear-gradient(135deg, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.25) 45%, rgba(255,255,255,0.08) 70%, rgba(255,255,255,0.02) 100%)', mixBlendMode: 'screen' }}></span>
        {/* Label layer */}
        <div className="absolute inset-0 flex items-center justify-center">
          <span className={`${compact ? 'text-sm' : 'text-base md:text-lg'} flex items-center gap-2 font-semibold text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.6)]`}>
...
export default function GitHubButton({ compact = false, className = '', autoAnimate = true, label = 'Github' }: { compact?: boolean; className?: string; autoAnimate?: boolean; label?: string }): JSX.Element {
